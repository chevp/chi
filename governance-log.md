...
2026-05-02 | chevp | gate-pass | G1 | CTX-001 worktree parallelization approved: H1 (thin git-worktree wrapper) + H2 (per-worktree GIT_DIR isolation), R1-R4 mitigated, scope locked to `chi work` namespace + `chi done` cleanup hook (no changes to flow/issue fix defaults). User direction: skip G2 (no exploration prototype); H2 smoke test (parallel chi ship from two worktrees) moved to G3 acceptance.
2026-05-02 | chevp | gate-override | G2 | CTX-001 → G3 direct, by user request. Risk: H2 unvalidated until first parallel `chi ship`. Rollback plan: revert PR if smoke test fails.
2026-05-02 | chevp | adr-accepted | ADR-006 | Worktrees as parallelism primitive: `chi work` namespace wraps `git worktree`, per-worktree marker at `<common>/worktrees/<name>/chi-worktree`, `chi done` cleans up worktree on merge. Symlinked node_modules + auto-launch deferred.
2026-05-02 | chevp | scope-expansion | CTX-001 | User direction: extend CTX-001 v1 scope to also auto-route `chi issue fix <N>` through a worktree (was explicitly out-of-scope per the original CTX). Branch naming preserved (`fix/issue-N`); worktree path `<repo>-issue-N`; flow marker written directly by issue-fix script (replaces `chi flow` call). New task; H2 (parallel ship) still pending validation.
