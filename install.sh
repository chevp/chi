#!/usr/bin/env bash
# chi installer for macOS / Linux / WSL.
#
# Builds the project in-place and symlinks bin/chi into PREFIX/bin
# so that `chi` is available on PATH.
#
# Usage:
...
for arg in "$@"; do
  case "$arg" in
    -y|--yes)         CHI_ASSUME_YES=1 ;;
    --no-path-edit)   CHI_NO_PATH_EDIT=1 ;;
    -h|--help)
...
if ! command -v node >/dev/null 2>&1; then
  ...
  echo "chi requires Node.js 20+: https://nodejs.org/" >&2
  exit 1
fi

node_major="$(node -e 'process.stdout.write(String(process.versions.node.split(".")[0]))')"
if [ "$node_major" -lt 20 ] 2>/dev/null; then
  echo "error: node $node_major found, but chi requires node 20+" >&2
  exit 1
fi
...
# 2. Build
# ---------------------------------------------------------------------------
printf "${C_BOLD}chi install${C_RESET}\n"
...
# ---------------------------------------------------------------------------
# 3. Symlink bin/chi into PREFIX/bin
# ---------------------------------------------------------------------------
mkdir -p "$PREFIX/bin"
ln -sf "$SRC/bin/chi" "$PREFIX/bin/chi"

_short_sha="unknown"
if git -C "$SRC" rev-parse --git-dir >/dev/null 2>&1; then
  _short_sha="$(git -C "$SRC" rev-parse --short HEAD 2>/dev/null || printf unknown)"
fi
printf "${C_BOLD}chi${C_RESET} → %s  ${C_DIM}(%s)${C_RESET}\n" "$PREFIX/bin/chi" "$_short_sha"
...
case ":$PATH:" in
  ...
  *)
    if [ -n "$shell_rc" ] && [ "${CHI_NO_PATH_EDIT:-0}" != "1" ]; then
      mkdir -p "$(dirname "$shell_rc")"
      touch "$shell_rc"
      if ! grep -Fqs "$export_line" "$shell_rc"; then
        {
          echo ""
          echo "# added by chi install.sh"
          echo "$export_line"
        } >> "$shell_rc"
...
# 5. Verify
# ---------------------------------------------------------------------------
if [ -x "$PREFIX/bin/chi" ]; then
  PATH="$PREFIX/bin:$PATH" chi status >/dev/null 2>&1 && printf "${C_DIM}verify   ${C_RESET}${C_GREEN}ok${C_RESET}\n"
fi

printf "\n${C_GREEN}→ ready.${C_RESET}  next: ${C_BOLD}chi status${C_RESET}\n"
