...
function isExecutionError(out: string): boolean {
 ...
 */
function repairPluginsFile(): boolean {
  ...
  const root = parsed as { plugins?: Record<string, unknown> };
    ...
    if (isExecutionError(out)) {
      if (repairPluginsFile()) {
        process.stderr.write(
          "claude-code: repaired malformed ~/.claude/plugins/installed_plugins.json (backup at .bak), retrying\n",
        );
        out = await callClaude(prompt);
      }
      if (isExecutionError(out)) {
        throw new Error(
          'claude CLI returned "Execution error" — run \'claude -p --output-format json "hi"\' to inspect the underlying error',
        );
      }
    }
    return out;
