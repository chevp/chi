...
export function checkForUpdate() {
    if (process.env.CHI_NO_UPDATE_CHECK === "1")
        return;
    ...
    const fetch = spawnSync("git", ["-C", chiRoot, "fetch", "--quiet", "origin", "main"], { encoding: "utf8", windowsHide: true, timeout: 5000 });
    if (fetch.status !== 0)
        return;
    const local = spawnSync("git", ["-C", chiRoot, "rev-parse", "HEAD"], { encoding: "utf8", windowsHide: true });
    const remote = spawnSync("git", ["-C", chiRoot, "rev-parse", "origin/main"], { encoding: "utf8", windowsHide: true });
    ...
    // on a feature branch).
    const behind = spawnSync("git", ["-C", chiRoot, "merge-base", "--is-ancestor", localSha, remoteSha], { windowsHide: true });
    if (behind.status !== 0)
        return;
