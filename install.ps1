# chi installer for Windows PowerShell.
#
# Builds the project in-place and adds bin/ to the user PATH
# so that `chi` is available in new terminals.
#
# Usage:
...
Write-Host "chi install" -ForegroundColor White
...
try {
    ...
    Write-Error "node is not installed or not on PATH. chi requires Node.js 20+: https://nodejs.org/"
    exit 1
}
if (-not $nodeVersion) {
    Write-Error "node is not installed or not on PATH. chi requires Node.js 20+: https://nodejs.org/"
...
if ($nodeMajor -lt 20) {
    Write-Error "node $nodeVersion found, but chi requires node 20+"
    exit 1
}
...
$verified = $false
try {
    $chiBin = Join-Path $binDir 'chi.cmd'
    if (Test-Path $chiBin) {
        & $chiBin status 2>&1 | Out-Null
        if ($LASTEXITCODE -eq 0) { $verified = $true }
    }
} catch { }

if ($verified) {
    Write-Host "  [OK]   chi status works" -ForegroundColor Green
} else {
    Write-Host "  [INFO] run 'chi status' manually after opening a new terminal" -ForegroundColor DarkGray
}

Write-Host ""
Write-Host "done. open a new terminal and try: chi status" -ForegroundColor Green
